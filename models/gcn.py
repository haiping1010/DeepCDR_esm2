...
class GCNNet(torch.nn.Module):
    ...
    def __init__(self, n_output=1, n_filters=64,n_heads=6,n_layers=4, embed_dim=1280,num_features_xd=20, num_features_xt=20, output_dim=1280, dropout=0.2):
    ...
    def forward(self, data, TRAIN_BATCH_SIZE, device):
        ...
        target = data.target.reshape(-1,95,20)
        ...
        xt=target.to(device)
        #print (xt.shape)
        conv_xt = self.conv_xt_1(xt)
        # flatten
        xt = conv_xt.view(-1, 64*88)
        xt = self.fc1_xt(xt)
