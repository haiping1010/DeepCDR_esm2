...
with open("amino_acid_vectors_20dim.pkl", "rb") as f:
...
def  pdb_graph(pdbfile):
  ...
  for index, item in enumerate(uniq_n):
        #print (item)
        feature = aa_dict[aa_mapping[item[0:3]]]
...
reverse_mapping = {value: key for key, value in aa_mapping.items()}
#######
def seq_cat(prot):
    # 初始化一个零矩阵，形状为 (max_seq_len, vector_dim)
    vector_dim = 20  # 预训练向量的维度
    x = np.zeros((max_seq_len, vector_dim))
    
    for i, ch in enumerate(prot[:max_seq_len]):
        if ch in aa_dict:
            x[i] = aa_dict[ch]
        else:
            x[i] = aa_dict['X']  # 使用未知氨基酸的向量
    return x

#seq_voc = "ABCDEFGHIKLMNOPQRSTUVWXYZ"
seq_voc = "ACDEFGHIKLMNPQRSTVWYX"
aa_dict['X']=np.zeros(20)
max_seq_len = 95

#aa_dict_s = {aa_mapping[k]: v for k, v in aa_dict.items()}
...
for name in arr_frr:
   print (name.strip(),list(sequences_dict.keys())[1])
   arr_tem=name.split()
   if   name.replace('.pdb','_CDR.txt').strip()  in   sequences_dict.keys():
...
class TestbedDataset(InMemoryDataset):
    ...
    def process(self, xd, pocket1_graph, y, pocket2_graph):
        ...
        for i in range(data_len):
            ...
            labels=y[i]
            print (filename)
            #arr_f=filename.strip().split()
            # convert SMILES to molecular representation using rdkit
            # make the graph ready for PyTorch Geometrics GCN algorithms:
            #c_size, features, edge_index =  smile_graph[str(filename[0:4])]
            #fr=open(arr_f[0],'r')
            #print (linearr[0])
            c_size, features, edge_index =  pdb_graph('xxxx_antigen_poc.pdb')
            ...
            GCNData.name = filename.replace('antibody.pdb','')
            #GCNData.target = DATA.Data(x=torch.Tensor(features1),
            #                    edge_index=torch.LongTensor(edge_index1).transpose(1, 0))
            t=sequences_dict[filename.replace('.pdb','_CDR.txt')]
            XT=seq_cat(t)
            GCNData.target = torch.tensor(XT, dtype=torch.float32).reshape(95, 20)
            print (GCNData.target.shape)
...
train_data = TestbedDataset(root='data1', dataset='L_P_test_'+idx, xd=arr_name, pocket1_graph=pocket1_graph, y=train_Y, pocket2_graph=pocket2_graph)
